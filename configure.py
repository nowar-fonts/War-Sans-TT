...
class Config:
	...
	copyright = "Copyright © 2018—2020 Cyano Hao and Nowar Typeface. Portions Copyright 2015 Google Inc. Portions © 2014-2019 Adobe (http://www.adobe.com/)."
	...
	fontPackRegion = [ "CN", "TW", "HK", "JP", "KR", "CL", "GBK" ]
...
def LocalizedFamily(p):
	if "nameList" not in LocalizedFamily.__dict__:
		LocalizedFamily.nameList = {
			...
			"WarcraftSans": {
				0x0409: "War Sans Warcraft",
			},
			"WarcraftUI": {
				0x0409: "War UI Warcraft",
			},
		}
...
def GenerateFilename(p):
	if p.family in [ "Sans", "UI", "WarcraftSans", "WarcraftUI" ]:
		...
		nameList = {
			"Sans": "WarSans",
			"UI": "WarUI",
			"WarcraftSans": "WarSansWarcraft",
			"WarcraftUI": "WarUIWarcraft",
		}
...
if __name__ == "__main__":
	...
	for r, w, fea in product(config.fontPackRegion, config.fontPackWeight, powerset(config.fontPackFeature)):
		...
		pack = "out/WarSans-{}-${{VERSION}}.7z".format(target)
